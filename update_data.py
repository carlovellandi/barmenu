...
def fetch_harvard_espn():
    ...
    for path, label in HARVARD_SPORTS:
        urls = [
            ...
            "https://site.api.espn.com/apis/site/v2/sports/%s/teams/%s/schedule" % (path, HARVARD_ID),
        ]
        data = None
        for url in urls:
            try:
        ...
        for ev in data.get("events", []):
            ...
            games.append({
                "sport": label,
                "opp": (them.get("team") or {}).get("shortDisplayName") or (them.get("team") or {}).get("displayName") or "",
...
def fetch_harvard_rss():
    ...
    for it in items:
        ...
        games.append({
            "sport": sport.strip(), "opp": opp, "home": home, "state": state, "detail": detail,
